...
def simple_model():
    model = nn.Sequential(
        nn.Conv1d(in_channels=1, out_channels=128, kernel_size=50, stride=25, bias=False), # change output_channels from 32 to 128, change stride from 6 to 25
        nn.BatchNorm1d(num_features=128, eps=0.001, momentum=0.01), # change num_features from 32 to 128
        nn.ReLU(inplace=True),
        nn.MaxPool1d(kernel_size=8, stride=8),
        nn.Dropout(p=0.5),
        nn.Conv1d(in_channels=128, out_channels=128, kernel_size=8, stride=1, bias=False), # change in_channels from 32 to 128, change out_channels from 64 to 128
        nn.BatchNorm1d(num_features=128, eps=0.001, momentum=0.01), # change num_features from 64 to 128
        nn.ReLU(inplace=True),
        nn.MaxPool1d(kernel_size=4, stride=4), # change kernel_size and stride from 8 to 4
        nn.Dropout(p=0.5),
        nn.Flatten(),
        nn.Linear(in_features=128, out_features=5, bias=False) # change in_features from 384 to 128
    )
    return model
...
class SimpleModel:
    ...
    def __init__(self, config, device):
        ...
        self.loss = nn.CrossEntropyLoss(reduction='none')  # Use reduction='none' instead of reduce=False
        self.global_epoch = 0
        self.global_step = 0
        self.device = device
        self.config = config
    ...
    def train(self, minibatch_fn):
        ...
        for x, y, w, sl, re in minibatch_fn:
            ...
            y = torch.from_numpy(y).to(self.device).long()  # Ensure labels are long integers
